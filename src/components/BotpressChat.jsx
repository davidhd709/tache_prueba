...
export function BotpressChat() {
  useEffect(() => {
    const injectUrl =
      import.meta.env.VITE_BOTPRESS_INJECT_URL ||
      'https://cdn.botpress.cloud/webchat/v3.6/inject.js'
    const configUrl = import.meta.env.VITE_BOTPRESS_CONFIG_URL?.trim()
    const shareableUrl = import.meta.env.VITE_BOTPRESS_SHAREABLE_URL?.trim()
    let resolvedConfigUrl = configUrl

    if (!resolvedConfigUrl && shareableUrl) {
      try {
        const parsedUrl = new URL(shareableUrl)
        resolvedConfigUrl = parsedUrl.searchParams.get('configUrl') || ''
      } catch {
        resolvedConfigUrl = ''
      }
    }
